...
def computeLikelihoodPdf(kdePdf,gaugePdf,x,wt,offShoreHeights):
    ...
    if np.abs(intGaugePdf-1.0) > 0.05:
        print("WARNING: Integration rule may not be accurate enough. Integration of gauge PDF yielded: {:.6f}".format(intGaugePdf));
...
#each gauge
def makeGaugeKDEs(gauges,flNm=amp_data_path):
  heightKdes     = []
  inundationKdes = []
...
def buildGaugeLikelihoods(gaugeFile=gauges_path, tohokuFile=amp_data_path, heightFile=height_llh_path, inundationFile=inun_llh_path,offshorePoints=500,gaugeIds=-1):

    #there has *got* to be a better way to get gauges in the right format than to read, write, and read back in again. but this is what the main code does so I'm just mimicking it for now
    ...
    maxOffShoreHeight = max([ kde.dataset[1,:].max() for kde in heightKdes ]);#max([ val[:,1].max() for val in vals ]);
    ...
    for gid, gauge in enumerate(gauges):
        ...
        if(gauge.kind[2]):
            inundationLikelihood[:,gid], condDist = computeInundationLikelihoodPdf(inundationKdes[gid],gauges[gid],offShoreHeights);
