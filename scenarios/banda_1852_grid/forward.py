...
class VanillaNet(torch.nn.Module):
    def __init__(self, input_size, hidden_size, output_size, scalers=None, retain_graph=False):
        ...
        self.hidden_layers = torch.nn.Sequential(
            *chain.from_iterable(
                [torch.nn.Linear(a, b), torch.nn.ReLU()] for a, b in
...
class NeuralNetEmulator(BaseForwardModel):
    ...
    def run(self, model_params, raw_output=False, verbose=False):
    ...
    def llh(self, model_output, verbose=True):
...
#                 llh += log_p
#                 if verbose:
#                     print("inundation: {:.3f}\tllh: {:.3e}".format(
#                         inundation, log_p))
        return llh

    def compute_gradient(self, model_params_in):
        """Computes and returns the gradient of the nerual network with respect
        to each network input parameter
        ...
        -------
        grads : dict
            The gradient of each height output by the neural network with respect
            to its inputs. It is a dictionary whose keys are the output heights
            'Pulu Ai height', 'Ambon height', 'Banda Neira arrival',
            'Banda Neira height', 'Buru height', 'Hulaliu height',
            'Saparua arrival', 'Saparua height', 'Kulur height', 'Ameth height',
            'Amahai height'. The value at each key is a pd.Series object whose keys are
            the neural network inputs 'latitude', 'longitude', 'length', 'width', 'slip',
            'strike', 'dip', 'depth' and whose values are the derivative of the
            corresponding height with respect to the key value.
        """
        model_params = model_params_in.iloc[-1]
        model_params['depth'] += model_params['depth_offset']
        X = torch.tensor([model_params[x] for x in self.nn_input_cols])
...
#         print(X)
#         print(X.shape)
        output = self.net.forward_unnormalized(X.float())

        model_output = pd.Series(dtype='float64')
#         print(output)
        for idx, output_col in enumerate(self.nn_output_cols):
            model_output[output_col] = output[idx].item()

        grad_matrix = torch.zeros(
            (len(self.nn_output_cols), len(self.nn_input_cols)))

        for i in range(grad_matrix.shape[1]):
            grad_matrix[i] = torch.autograd.grad(
                output[i], X, grad_outputs=torch.ones_like(output[i]), retain_graph=True)[0]

        grads = {}
        for i, col_name in enumerate(self.nn_output_cols):
            current_grad = pd.Series(grad_matrix[i])
            current_grad.index = self.nn_input_cols

            grads[col_name] = current_grad

        return grads, model_output
    ...
    def write_fgmax_grid(self, gauges, fgmax_params):
        ...
        with open(fgmax_params['fgmax_grid_path'], 'w') as f:
            ...
            f.write(str(fgmax_params['min_level_check']
                        ) + '\t# min_level_check\n')
