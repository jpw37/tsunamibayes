import os
print(f"os.environ['CONDA_DEFAULT_ENV']")
import numpy as np
import pandas as pd
...
def centered_diff(config, step):
    ...
    for param in param_vals:
        ...
        print(fwd_params) 
        print(scenario.map_to_model_params(fwd_params))
        fwd_modelparams = scenario.map_to_model_params(fwd_params)
        bck_modelparams = scenario.map_to_model_params(bck_params)
        print(f'MODEL PARAMS BEING USED*****: {fwd_modelparams}')



        #print(forward_model.run(fwd_modelparams))
        print(gauge_vals)
        print('RUNNING GEOCLAW FORWARD')
        fwd_out = forward_model.run(fwd_modelparams)#[gauge_vals]
        bck_out = forward_model.run(bck_modelparams)#[gauge_vals]
        ...
        print(centered_diff)
        print(type(centered_diff.values))
        print(len(centered_diff.values))
        print(centered_diff.values.shape)
        model_df[param] = centered_diff.loc[gauge_vals].values
