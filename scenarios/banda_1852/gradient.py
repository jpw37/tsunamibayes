...
def gradient_setup(dip_map, depth_map, config, step=0.01):
    ...
    # Pretty sure this is TBF stuff
    def radius_lambda(gauge_lat, gauge_lon): return 2 * earth_rad * asin(sy.sqrt(
    ...
    for i, gauge in enumerate(gauges):
        ...
        theta, phi = to_rad(dip_map(gauge.lat, gauge.lon)), to_rad(
            90)  # TODO Look up actual dip and rake using lat and lon
        alpha = (1 - theta / 180) * sy.sin(theta) * Abs(sy.sin(phi))
        R = radius_lambda(gauge.lat, gauge.lon)

        # Combined formula for mean wave height (TBF formulas)
        denom = sy.cosh((4 * sy.pi * H_0) / (width + length))
        # This A value is what we will get from neural net
        A = ((alpha * slip) / denom) * sy.Pow(1 +
                                              (2 * R / length), -psi) * (H_0 / H)**(1 / 4)
        ...
        if i == 0:
            ...
            def dh_ddo(l1, l2, m, dll, dlw): return 0
