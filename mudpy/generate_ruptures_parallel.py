...
def run_parallel_generate_ruptures(home,project_name,run_name,fault_name,slab_name,mesh_name,
        ...
        max_slip_rule,zvals,stochastic_rake,rank,size):
...
#                        slip_unrectified,success=make_KL_slip(fault_array,num_modes,eigenvals,V,mean_slip,max_slip,lognormal=False,seed=kfault)
                        slip_unrectified,success=fakequakes.make_KL_slip(fault_array,num_modes,eigenvals,V,mean_slip,max_slip,zvals,lognormal=False,seed=None)
...
#                    slip,success=make_KL_slip(fault_array,num_modes,eigenvals,V,mean_slip_log,max_slip,lognormal=True,seed=kfault)
                    slip,success=fakequakes.make_KL_slip(fault_array,num_modes,eigenvals,V,mean_slip_log,max_slip,zvals,lognormal=True,seed=None)
...
if __name__ == '__main__':
    ...
    if sys.argv[1]=='run_parallel_generate_ruptures':
        ...
        if zvals=='False':
            ...
            zvals = zvals + sys.argv[42] + sys.argv[43]
            zvals = json.loads(zvals)
        stochastic_rake = sys.argv[44]
        if stochastic_rake == 'True':
            stochastic_rake = True
        if stochastic_rake == 'False':
