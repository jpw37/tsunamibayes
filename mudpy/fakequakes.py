...
it to work in a similar way to the MudPy inversions
'''
...
def get_mean_slip(target_Mw,fault_array,vel_mod):
    ...
    """Maybe chage it to the 9.05 value from the paper"""
...
def make_KL_slip(fault,num_modes,eigenvals,V,mean_slip,max_slip,zvals,lognormal=True,maxiter=5,seed=12345):
    '''
...
def generate_ruptures(home,project_name,run_name,fault_name,slab_name,mesh_name,
		...
		shear_wave_fraction_shallow=0.49,shear_wave_fraction_deep=0.8,max_slip_rule=True,zvals=False,stochastic_rake=True):
    '''
    ...
    no_random,shypo,use_hypo_fraction,shear_wave_fraction_shallow,shear_wave_fraction_deep,max_slip_rule,zvals,stochastic_rake)
...
def run_generate_ruptures(home,project_name,run_name,fault_name,slab_name,mesh_name,
        ...
        force_magnitude,force_area,mean_slip_name,hypocenter,slip_tol,force_hypocenter,
        no_random,shypo,use_hypo_fraction,shear_wave_fraction,max_slip_rule,zvals):
...
#                        slip_unrectified,success=make_KL_slip(fault_array,num_modes,eigenvals,V,mean_slip,max_slip,lognormal=False,seed=kfault)
                        slip_unrectified,success=make_KL_slip(fault_array,num_modes,eigenvals,V,mean_slip,max_slip,zvals,lognormal=False,seed=None)
...
#                    slip,success=make_KL_slip(fault_array,num_modes,eigenvals,V,mean_slip_log,max_slip,lognormal=True,seed=kfault)
                    slip,success=make_KL_slip(fault_array,num_modes,eigenvals,V,mean_slip_log,max_slip,zvals,lognormal=True,seed=None)
...
#            hypo_fault=ifaults[islip[0]] #select first from randomized vector
            ...
            #For Tsunamibayes purposes, stop here
            return fault_out
