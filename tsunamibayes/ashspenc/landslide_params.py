...
small_lst = np.linspace(1, 150, density, endpoint=True)
middle_lst = []
big_lst = []
...
for i in lref_lst:
    ...
    for row_index in random_idx:
        # time one landslides
        start_one_landslide = time.perf_counter()
        ...
        # time forward model
        start_toy_model = time.perf_counter()
        # toy = forward.ToyForwardModel(gauge_obj_lst, None, path)
        toy_results = toy.run(landslide_params, lref=i)
        print(f"time for {row_index}th toy model simulation is {round(time.perf_counter() - start_toy_model)} seconds.")
        ...
        print(f"time for {row_index}th landslide is {round(time.perf_counter() - start_one_landslide)} seconds.")
