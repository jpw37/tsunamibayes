...
def haversine(lat1, lon1, lat2, lon2):
    ...
    term = (
        np.sin(.5 * (phi2 - phi1))**2
        + np.cos(phi1) * np.cos(phi2) * np.sin(.5 * (lam2 - lam1))**2
    )
...
def displace(lat, lon, bearing, distance):
    """Compute the lat-lon coordinates of a point on a sphere after the
    displacement of a given point along a specified bearing and distance.
    R = radius of the earth. All of the following parameters must have the
    same dimension.
...
def calc_corners(subfault_params):
    """Compute the corners of the Okada rectangles specified in the paramters
    of a subfault.
    ...
    subfault_params : pandas DataFrame
        ...
        and whose rows contain the associated data (float values) for each
        subfault.

    Returns
    -------
    corners : ndarray
        A single stacked array containing the coordinate pairs (float values
        in degrees) for the four corners for the given Okada rectangles.
    """
...
def out_of_bounds(subfault_params, bounds):
    """Returns true if any subfault lies outside of the bounds, or intersects
    with the surface.
    ...
    for _,subfault in subfault_params.iterrows():
        if subfault['depth'] < (
            .5*subfault['width']*np.sin(np.deg2rad(subfault['dip']))
        ):
            return True
