...
def resample(output_dirs, overwrite=False,  verbose=False):
    samples = [pd.read_csv(dir_+"output/samples.csv", index_col=0) for dir_ in output_dirs]
    final_samples = [df.iloc[-1] for df in samples]
    model_params = [pd.read_csv(dir_+"output/model_params.csv", index_col=0) for dir_ in output_dirs]
    model_output = [pd.read_csv(dir_+"output/model_output.csv", index_col=0) for dir_ in output_dirs]
    bayes_data = [pd.read_csv(dir_+"output/bayes_data.csv", index_col=0) for dir_ in output_dirs]
    debug = [pd.read_csv(dir_+"output/debug.csv", index_col=0) for dir_ in output_dirs]
