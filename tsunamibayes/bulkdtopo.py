...
def compute_dtopos(fault,model_params,verbose=False):
    ...
    model_params : dict
        ...
        'width','slip','depth','dip','rake', and whose associated values are floats. 
    verbose : bool
        Flag for verbose output, optional. Default is False.
        If set to true, the function will print the index's iterated within the model_params.

    Returns
    -------
    dtopos : array_like of floats
        The ndarray containing the values of the seafloor deformation- in other words
        the changes of depth in the seafloor. This array must have dimesions: (len(x), len(y)). 
    x : array_like of floats
        The ndarray containing the x coordinates (longitude) for each point along the subfault.
    y : array_like of floats
        The ndarray containing the y coordinates (latitude) for each point along the subfault.
    """
